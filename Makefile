...
build-app: $(SPARKLE_FRAMEWORK)
	...
	CGO_ENABLED=1 CGO_LDFLAGS="$(APP_CGO_LDFLAGS)" \
		go build -tags sparkle -ldflags="-s -w -X main.buildVersion=$(APP_VERSION)" -o "$(APP_NAME)/Contents/MacOS/vekil-menubar" ./cmd/menubar/
...
build-tray-linux:
	CGO_ENABLED=0 GOOS=linux \
		go build -ldflags="-s -w -X main.buildVersion=$(APP_VERSION)" -o $(TRAY_LINUX_BINARY) ./cmd/menubar/
