...
func runLoginWithDeps(args []string, deps loginDeps) int {
	...
	if opts.useGitHubCLI {
		...
		loginInfof(opts, deps.stderr, "Login successful.\n")
		return 0
	}

	if !opts.force {
		if _, err := authenticator.RefreshTokenNonInteractive(ctx); err == nil {
			loginInfof(opts, deps.stderr, "Already logged in.\n")
			return 0
	...
	loginInfof(opts, deps.stderr, "Opening browser to %s\n", dcResp.VerificationURI)
	loginInfof(opts, deps.stderr, "Enter code: %s\n", dcResp.UserCode)

	if err := deps.openURL(dcResp.VerificationURI); err != nil {
		if opts.quiet {
			_, _ = fmt.Fprintf(deps.stderr, "Could not open browser automatically, please visit %s and enter code: %s\n", dcResp.VerificationURI, dcResp.UserCode)
		} else {
...
func loginInfof(opts loginOptions, w io.Writer, format string, args ...interface{}) {
	if opts.quiet {
		return
	}
