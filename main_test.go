...
func TestCommandFromArgs(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			args: []string{"placeholder", "version"},
...
func TestParseLaunchClaudeOptionsMetricsOptOut(t *testing.T) {
	t.Run("environment", func(t *testing.T) {
		t.Setenv("NO_METRICS", "true")
		var stderr bytes.Buffer
		opts, err := parseLaunchClaudeOptions([]string{"--model", "claude-public", "--dry-run"}, &stderr)
		if err != nil {
			t.Fatalf("parseLaunchClaudeOptions() error = %v", err)
		}
		if !opts.noMetrics {
			t.Fatal("NO_METRICS=true did not disable launch metrics")
		}
	})
	...
	t.Run("flag override", func(t *testing.T) {
		t.Setenv("NO_METRICS", "true")
		var stderr bytes.Buffer
		opts, err := parseLaunchClaudeOptions([]string{"--model", "claude-public", "--no-metrics=false", "--dry-run"}, &stderr)
		if err != nil {
			t.Fatalf("parseLaunchClaudeOptions() error = %v", err)
		}
		if opts.noMetrics {
			t.Fatal("--no-metrics=false did not override NO_METRICS=true")
		}
	})
}

func TestLaunchProxyOptionsApplyMetricsAndBuildInfo(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{name: "enabled", wantHandler: true},
		{name: "disabled", noMetrics: true},
	} {
		t.Run(tt.name, func(t *testing.T) {
			handler, err := proxy.NewProxyHandler(
				auth.NewTestAuthenticator("test-token"),
				logger.NewWithWriter(logger.LevelError, io.Discard),
				launchProxyOptionsWithBuildInfo(
					cfg,
					launchAgentOptions{model: "local-model", noMetrics: tt.noMetrics},
					"launch-version",
					"launch-commit",
					"go-launch",
				)...,
			...
			if (metricsHandler != nil) != tt.wantHandler {
				t.Fatalf("MetricsHandler() present = %v, want %v", metricsHandler != nil, tt.wantHandler)
			}
