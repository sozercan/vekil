...
// endpoint is mounted on the shared HTTP server.
...
func New(authenticator *auth.Authenticator, log *logger.Logger, host, port string, opts ...Option) (*Server, error) {
	...
	handle := func(pattern, label string, next http.HandlerFunc) {
		var wrapped http.Handler = http.HandlerFunc(next)
		if metrics != nil {
			wrapped = metrics.instrument(label, next)
		}
		mux.Handle(pattern, wrapped)
	}

	handle("POST /v1/messages", "messages", handler.HandleAnthropicMessages)
