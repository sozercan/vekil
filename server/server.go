...
func New(authenticator *auth.Authenticator, log *logger.Logger, host, port string, opts ...Option) (*Server, error) {
	...
	handle := func(pattern, name string, h http.HandlerFunc) {
		if metrics != nil {
			mux.Handle(pattern, metrics.instrumentHandler(name, h))
			return
		}
	...
	handle("POST /v1beta/models/", "gemini_models_v1beta", handler.HandleGeminiModels)
	handle("POST /v1/models/", "gemini_models_v1", handler.HandleGeminiModels)
	...
	handle("POST /v1/memories/trace_summarize", "memories_trace_summarize", handler.HandleMemorySummarize)
