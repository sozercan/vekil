...
func New(authenticator *auth.Authenticator, log *logger.Logger, host, port string, opts ...Option) (*Server, error) {
	...
	mux.Handle("POST /v1/messages/count_tokens", metrics.instrument(http.HandlerFunc(handler.HandleAnthropicMessagesCountTokens)))
	mux.Handle("POST /v1/messages", metrics.instrument(http.HandlerFunc(handler.HandleAnthropicMessages)))
	mux.Handle("POST /v1/chat/completions", metrics.instrument(http.HandlerFunc(handler.HandleOpenAIChatCompletions)))
	mux.Handle("POST /v1beta/models/", metrics.instrument(http.HandlerFunc(handler.HandleGeminiModels)))
	mux.Handle("POST /v1/models/", metrics.instrument(http.HandlerFunc(handler.HandleGeminiModels)))
	mux.Handle("POST /models/", metrics.instrument(http.HandlerFunc(handler.HandleGeminiModels)))
	mux.Handle("POST /v1/responses/compact", metrics.instrument(http.HandlerFunc(handler.HandleCompact)))
	mux.Handle("POST /v1/responses", metrics.instrument(http.HandlerFunc(handler.HandleResponses)))
	mux.Handle("GET /v1/responses", metrics.instrument(http.HandlerFunc(handler.HandleResponsesWebSocket)))
	mux.Handle("POST /v1/memories/trace_summarize", metrics.instrument(http.HandlerFunc(handler.HandleMemorySummarize)))
	mux.Handle("GET /healthz", metrics.instrument(http.HandlerFunc(handler.HandleHealthz)))
	mux.Handle("GET /readyz", metrics.instrument(http.HandlerFunc(handler.HandleReadyz)))
	mux.Handle("GET /v1/models", metrics.instrument(http.HandlerFunc(handler.HandleModels)))
