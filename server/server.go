...
// WithBuildVersion sets the version exposed in metrics.
...
func New(authenticator *auth.Authenticator, log *logger.Logger, host, port string, opts ...Option) (*Server, error) {
	...
	handle := func(pattern string, _ string, fn http.HandlerFunc) {
		mux.HandleFunc(pattern, fn)
	}
	if cfg.metrics {
		metrics := newMetrics(cfg.buildVersion)
		handle = func(pattern, route string, fn http.HandlerFunc) {
			mux.Handle(pattern, metrics.wrap(route, fn))
		}
		mux.Handle("GET /metrics", metrics.wrap("/metrics", metrics.handler()))
	}
	...
	handle("GET /v1/responses", "/v1/responses_ws", handler.HandleResponsesWebSocket)
