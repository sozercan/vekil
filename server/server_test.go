...
func TestNew_RegistersMetricsEndpointByDefault(t *testing.T) {
	srv, err := New(
		auth.NewTestAuthenticator("test-token"),
		logger.New(logger.ParseLevel("error")),
		"127.0.0.1",
	...
	if !strings.Contains(rec.Body.String(), "vekil_build_info{") {
		t.Fatalf("GET /metrics missing build info metric: %s", rec.Body.String())
	}
}
