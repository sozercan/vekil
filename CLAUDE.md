...
- `live-policy-routing-copilot-smoke.yaml` — default credentialed semantic-policy PR gate. It reuses `COPILOT_GITHUB_TOKEN` and runs the full policy harness through a controllable loopback bridge; production policy configs may also pin direct Copilot Chat or Responses-backed targets.
