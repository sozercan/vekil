...
- **Durable ownership defaults to one local writer**: Schema-v2 explicit routes use a private Linux or macOS APFS bbolt store configured through the providers file; `state_bindings.mode: memory` opts out. Capture the actual authenticated owner before dispatch and commit proof before exposure. Durable records never expire/evict automatically; capacity limits logical records, including tombstones, without preallocation. Explicit offline pruning breaks affected continuity. Preserve legacy/zero-config defaults and keep Chat replay/websocket connection history process-local. See `docs/state-recovery.md`.
