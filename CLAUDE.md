...
| `proxy/metrics.go`, `proxy/stats.go`, shared hooks in `proxy/request_summary.go` | Prometheus collection and `/metrics` serving, dashboard traffic aggregates, bounded metric labels, and request/attempt attribution |
