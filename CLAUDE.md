...
| `proxy/providers.go`, `proxy/model_routes.go`, `proxy/model_routes_config.go`, `proxy/route_executor.go`, `proxy/provider_endpoint_policy.go`, `proxy/upstream_http.go`, `proxy/openai_codex_auth.go`, `proxy/openai_codex_account_pool.go`, `proxy/azure_identity_auth.go` | Provider config, logical route ownership, ordered target execution, endpoint allowlists, model rewrite, provider auth, upstream request construction |
...
- **OpenAI Codex support is file-auth-backed provider routing**: Codex models use CLI ChatGPT auth files, dynamic `/models` discovery, and `/responses`-only routing. An opt-in provider-internal account pool may select among explicitly configured auth files after route selection while preserving exact credential ownership for continuation state.
