...
- **Durable ownership is opt-in and single-writer**: Explicit-route state can use a private Linux local bbolt store. Capture the actual authenticated owner before dispatch and commit proof before exposure. Durable records never expire/evict automatically; capacity limits logical records, including tombstones, and explicit offline pruning breaks affected continuity. Preserve memory-only defaults and keep Chat replay/websocket connection history process-local. See `docs/state-recovery.md`.
