...
func TestConversationMigrationRejectsHostedCatalogsAndInvalidMetadata(t *testing.T) {
	var sends atomic.Int32
	...
	for _, scenario := range []string{"hosted", "nested hosted", "non-string metadata", "oversized metadata", "catalog capacity"} {
		t.Run(scenario, func(t *testing.T) {
			...
			wantCode := "conversation_state_unsupported"
			switch scenario {
			case "hosted":
				catalog["tools"] = []any{map[string]any{"type": "code_interpreter", "container": "foreign"}}
			case "nested hosted":
				catalog["tools"].([]any)[0].(map[string]any)["tools"] = []any{map[string]any{"type": "web_search"}}
			...
			case "catalog capacity":
				...
				wantCode = "conversation_history_capacity_exceeded"
			}
			result := conversationPOST(t, h, fields, nil)
			if result.Code < 400 || !strings.Contains(result.Body.String(), wantCode) || sends.Load() != 0 {
				t.Fatalf("invalid request was dispatched: %d %s sends=%d", result.Code, result.Body.String(), sends.Load())
			}
		})
	}
}
