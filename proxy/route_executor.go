...
func (h *ProxyHandler) executeExplicitRouteRequestPath(ctx context.Context, route *modelRoute, endpoint, dispatchPath string, body []byte, extraHeaders http.Header, requestedModel string, stream bool) (*http.Response, error) {
	...
	for {
		...
		// The logical inbound context owns HTTP request-summary attribution, while
		// the detached request context may own a websocket attempt observer. Mark
		// both at the physical dispatch boundary; repeated true stores are harmless.
		observeUpstreamAttempt(operation.inbound)
