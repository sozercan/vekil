...
func responsesRetrySettings(h *ProxyHandler) (int, time.Duration) {
...
func responsesRetryDelayWithRetryAfter(base time.Duration, attempt int, retryAfter string) time.Duration {
	delay := backoff(base, attempt)
	if ra, ok := parseRetryAfter(strings.TrimSpace(retryAfter)); ok && ra > delay {
		delay = ra
	}
	return delay
}

func (h *ProxyHandler) prepareRetriedResponsesStream(waitCtx, retryCtx context.Context, model string, request func() (*http.Response, error)) (*http.Response, *peekResult, http.Header, error) {
	maxRetries, retryDelay := responsesRetrySettings(h)
	if retryCtx == nil {
		retryCtx = waitCtx
	}
	if retryCtx == nil {
	...
	for attempt := range maxRetries {
		resp, result, translatedHeaders, err := prepareResponsesStreamAttempt(waitCtx, retryCtx, request)
		if err != nil || result == nil {
			return resp, nil, nil, err
		}
		if result.decision == responsesPeekDecisionTranslate {
			if attempt < maxRetries-1 {
				delay := responsesRetryDelayWithRetryAfter(retryDelay, attempt, result.retryAfter)
				h.log.Info("retrying responses stream after transient precommit failure",
					logger.F("endpoint", "responses_precommit_retry"),
					...
					logger.F("retry_delay_ms", delay.Milliseconds()),
					logger.F("attempt", attempt+1),
					logger.F("max_attempts", maxRetries),
					logger.F("upstream_request_id", responsesUpstreamRequestID(translatedHeaders)),
					logger.F("model", model),
				)
				if err := sleepWithContext(retryCtx, delay); err != nil {
					return nil, nil, nil, err
				}
				continue
			}
		...
		if result.failure != nil && resp != nil && result.decision == responsesPeekDecisionPassthrough {
			logResponsesPrecommitFailOpen(h, result.failure, model, resp.Header)
		}
		return resp, nil, nil, nil
	}
