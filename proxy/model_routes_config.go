...
func validateAndNormalizeProvidersConfig(cfg ProvidersConfig) (validatedProvidersConfig, error) {
	...
	providers, providerOrder, err := validateProviderConfigDescriptors(validated.config.Providers)
	if err != nil {
	...
	publicIDs := make(map[string]string, len(validated.config.ModelRoutes)*2)
	totalTargets := 0
	for routeIndex := range validated.config.ModelRoutes {
		...
		if err := normalizeAndValidateModelRoute(route, routePath); err != nil {
			return validatedProvidersConfig{}, err
		}
...
func cloneProvidersConfigForValidation(cfg ProvidersConfig) ProvidersConfig {
	...
	if cfg.Providers != nil {
		cloned.Providers = append([]ProviderConfig(nil), cfg.Providers...)
	}
...
func validateProviderConfigDescriptors(configured []ProviderConfig) (map[string]providerConfigDescriptor, []providerConfigDescriptor, error) {
	...
	for index, provider := range configured {
		...
		if prior, exists := providers[id]; exists {
			return nil, nil, configPathError(path+".id", "duplicates providers[%d].id", prior.index)
		}
		...
		default:
			return nil, nil, configPathError(path+".type", "unsupported provider type %q", provider.Type)
		}

		discovery, err := configuredProviderModelDiscovery(kind, provider.ModelDiscovery)
		if err != nil {
			return nil, nil, configPathError(path+".model_discovery", "%v", err)
		}
		...
		if kind == providerTypeCopilot {
			copilotCount++
			if copilotCount > 1 {
				return nil, nil, configPathError(path+".type", "multiple copilot providers are not supported")
			}
		}
		if provider.Default {
			if defaultIndex >= 0 {
				return nil, nil, configPathError(path+".default", "multiple default providers configured; providers[%d].default is already true", defaultIndex)
			}
			defaultIndex = index
		}
	}
	...
	if len(configured) > 1 && defaultIndex < 0 && copilotCount != 1 {
		return nil, nil, configPathError("providers", "multiple providers are configured but no default provider is selected")
	}
	return providers, ordered, nil
}
...
func normalizeAndValidateModelRoute(route *ModelRouteConfig, path string) error {
	...
	if route.PublicID = strings.TrimSpace(route.PublicID); route.PublicID == "" {
...
var topLevelProviderConfigFields = configFieldSet(
	"schema_version", "providers", "model_routes", "tool_optimizers", "insight_model",
)
...
var providerConfigFields = configFieldSet(
	...
	"models_path", "model_discovery", "headers", "models",
)
...
var modelRouteConfigFields = configFieldSet(
	"id", "public_id", "name", "endpoints", "reasoning_effort", "parallel_tool_calls", "vision",
