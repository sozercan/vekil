...
> **Policy-routing boundary:** schema-v2 policy profile IDs are accepted by a bounded stateless `POST /v1/responses` adapter that converts text, bounded `text.format` structured output, and function/namespace-child tools to canonical Chat. It requires `store: false` or omitted and rejects `previous_response_id`, hosted/custom tools, images, the websocket bridge, `/v1/responses/compact`, and `/v1/memories/trace_summarize`. Lightweight/powerful destinations may execute canonical Chat through native `/chat/completions` or bounded Chat-over-Responses; existing direct-route Responses behavior is unchanged.

## `POST /v1/responses` and `GET /v1/responses` (OpenAI)

`POST /v1/responses` is a near zero-copy passthrough for the OpenAI Responses API. Proxy-owned synthetic compaction items are expanded back into normal context before forwarding so resumed Codex sessions continue through the standard `/v1/responses` path. When a follow-up carries both a proxy-generated response ID (`resp-vekil-compact-*`) and its proxy-owned checkpoint, Vekil removes that synthetic `previous_response_id` and stale `X-Codex-Turn-State` before forwarding because neither value exists upstream; ordinary upstream response IDs and turn state remain unchanged. Responses Lite `additional_tools` namespace entries with an explicitly blank string `description` are normalized to `Tools in the <name> namespace.` before HTTP or proxy-owned websocket forwarding because upstream Responses validation requires a nonempty description. A missing, blank, non-string, or unusually long namespace name uses the bounded fallback `Tools in this namespace.`; missing, non-string, and nonempty descriptions remain unchanged. For provider routes other than the OpenAI Codex backend, Vekil also removes Codex client-only `internal_chat_message_metadata_passthrough` fields from `input` items before forwarding because Copilot, Azure, and compatible upstreams reject that internal metadata as an unknown Responses parameter.

A user message after the latest checkpoint suppresses the synthetic resume prompt, including a Responses input message that supplies `role` and `content` without `type`. User messages before that checkpoint are historical context and do not suppress resumption.
...
Every successful same-route response and stream event uses the configured canonical `model_routes[].public_id`; an accepted normalized alias is a request convenience and is not necessarily echoed. Failed-attempt response headers and request IDs are not merged into a later success. `X-Vekil-Request-ID` identifies the logical client operation; individual upstream request IDs remain attempt-level diagnostics.

### Exact Responses state binding

`previous_response_id`, Responses `conversation` IDs, trusted server-issued `X-Codex-Turn-State`, non-proxy opaque `encrypted_content`, and other adapter-marked opaque response/session artifacts are exact target state. Vekil binds provider-issued values to `{route_id, target_id}` immediately before the header, event, frame, or body content exposing them is committed. Hidden state from an abandoned precommit attempt is not stored; state already exposed remains bound even if the selected target later fails.

Explicit Responses requests reject duplicate JSON object keys and differing repeated `X-Codex-Turn-State` values before routing, and `previous_response_id` cannot be combined with `conversation`. All state supplied on a follow-up must agree on one owner. Known state routes to that exact target with no failover. Malformed, conflicting, cross-route, or mixed known/unknown state fails locally before any upstream call. If the owning target is unavailable, the request fails closed rather than guessing another target. Proxy-owned `resp-vekil-compact-*` checkpoints remain distinguishable from upstream state and continue to use the existing expansion behavior.

An unknown client-supplied `conversation` ID has one safe bootstrap path: the conversation must be the request's only explicit state, and the route must have exactly one eligible Responses target or use `primary_only` with an eligible deterministic primary. Vekil atomically binds the ID to that exact target before dispatch, then hard-pins the operation; later requests use the stored owner even if another target would otherwise be first. A multi-target `priority_failover` route rejects an unknown conversation before dispatch because the owner is ambiguous. Every other unknown provider-state value remains fail-closed. There is no public conversations endpoint or client target hint that bypasses this rule.

Schema-v2 explicit routes use [durable ownership](state-recovery.md) by default,
configured in the providers file. The default capacity is 8,388,608 retained
records without preallocation, automatic expiry, or eviction. Durable ownership
validates the exact authenticated owner and retains proof across restart; it
cannot recover already-lost proof.

`state_bindings.mode: memory` opts out and retains the process-local index with
a default 262,144-entry limit and 24-hour absolute TTL. Expiry, eviction, restart,
or another process makes that proof unknown. A conversation-only request on a
deterministic route can bootstrap again, so keep affinity and its target stable.
Concurrent clients can share one proxy, but each database has one writer and
separate proxies need ingress affinity. Shared storage and cross-target state
...
The Chat adapter is intentionally narrower than direct Responses passthrough. It accepts only the strict field subset in [API Reference](api.md#responses-backed-chat-request-subset), supports function tools only, rejects hosted tools, and rejects non-empty `stop` rather than emulating stop matching locally. Tool continuations use `call_vekil_...` IDs, self-describing when the upstream call ID can be embedded and opaque otherwise, together with a separate one-hour, byte-bounded, process-local replay store. A carrier preserves encrypted reasoning and ordering but does not serialize a separate upstream mapping. Replay state is unrelated to direct Responses `previous_response_id`, compaction checkpoints, or websocket history and is lost when Vekil restarts.
...
Requests to this endpoint are accepted up to `64 MiB` so large session histories can be compacted without tripping the default request-body limit. Internal compaction calls preserve model/provider/routing fields, but remove caller tools and tool selection (including root `additional_tools` input catalog items used by Responses Lite), streaming/background controls, structured-output constraints (`text.format` and `response_format`), and caller output-token caps so checkpoint generation cannot be diverted into a tool call, schema-only response, or trivially truncated output. Ordinary non-compaction `/v1/responses` forwarding preserves `additional_tools` apart from the blank namespace-description normalization described above. Vekil replaces the caller cap with a proxy-owned `max_output_tokens` limit that starts at `16,384` so internal summaries cannot inherit an unbounded provider default. When a provider explicitly rejects that value as above the selected model's maximum, Vekil halves the internal cap and retries down to the Responses minimum of `16`; the smallest discovered cap is reused by later chunks and the final merge in the same compaction fanout. A successful upstream summary body is buffered up to `1 MiB`, must be `completed` when a status is present, and must contain nonempty message output text; incomplete, refusal-only, function-call-only, empty, or oversized results fail without emitting a proxy checkpoint.
