...
The binding index is process-local, capped at 262,144 entries, and uses a 24-hour absolute TTL. Eviction, expiry, restart, or landing the follow-up on another Vekil process makes a prior binding unknown. A conversation-only request on a currently deterministic route can bootstrap again, so the proxy cannot distinguish genuine first use from ownership lost with the process-local index; keep both process affinity and the deterministic target stable while conversations are active. Every explicit route that accepts provider state—including one-target and `primary_only` routes—therefore requires one process or sticky ingress. There is no cross-target response-state migration, full-replay portability claim, durable/shared binding store, or circuit-breaker-driven target substitution.
