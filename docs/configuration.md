...
Remote configs are fetched once when the server, validator, or managed launcher loads them. Each fetch has a 15-second total timeout and a 4 MiB response-body limit; Vekil does not poll the URL or reload changes. Prefer HTTPS because an HTTP response can be modified in transit, and keep credentials in `api_key_env` rather than embedding them in remotely hosted configuration. Startup and validation fail closed when the source cannot be fetched or returns a non-2xx status.
