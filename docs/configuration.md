...
When enabled, `/metrics` stays mounted on the main HTTP server and exports Prometheus text exposition only when the request arrives directly from loopback (`127.0.0.1` / `::1`) and does not carry common forwarding headers such as `Forwarded`, `X-Forwarded-For`, `X-Real-IP`, or `Via`. Requests that are non-local or appear to have been proxied receive `403 Forbidden`. This is a hardening measure for shared-listener deployments so a same-host reverse proxy, sidecar, or tunnel cannot expose `/metrics` simply by connecting from loopback while preserving forwarding headers. The endpoint includes Go runtime/process collectors, `vekil_build_info`, and a small bounded `vekil_http_requests_total` counter for `/healthz`, `/readyz`, and `/v1/models`. Broader per-provider or payload-derived handler metrics are intentionally deferred in this first PR to avoid unbounded label cardinality and accidental leakage of user, key, or prompt content into labels.

This protection boundary is intentionally conservative but not a substitute for deployment isolation: if you publish Vekil behind a same-host proxy that strips forwarding headers, `/metrics` can still become reachable through that proxy. In that topology, disable metrics with `--no-metrics` / `METRICS=false` or keep the Vekil listener itself private to local scrapers.
