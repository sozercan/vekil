...
The default index is process-local, bounded to 262,144 entries with a 24-hour absolute TTL. Eviction, expiry, restart or another Vekil process makes prior proof unknown. Ordinary opaque state then fails closed; conversation-only IDs retain the narrow bootstrap exception. Lookups update eviction recency but not TTL; observing the same token from its owner refreshes it. Optional [durable state recovery](state-recovery.md) retains ownership across restart, binds the actual authenticated endpoint/deployment/account identity, and refuses new records at capacity without automatic expiry/eviction. It cannot reconstruct already-lost proof. Every explicit stateful route, including one-target and `primary_only`, still needs one writer or sticky ingress. Responses-backed Chat replay remains a separate process-local store. There is no shared binding store, cross-target replay/session migration or client-supplied ownership hint.
...
With durable ownership enabled, restarting against the complete same store and exact owner configuration preserves its proof, including bindings to a secondary. Websocket connections and Chat replay still need draining. Durable records have no TTL: the 24-hour waiting guidance above applies only to memory mode. Fence continuations before explicit pruning, disabling durable mode, restoring an older backup, or downgrading; see [recovery and rollback](state-recovery.md#failures-and-rollback).
