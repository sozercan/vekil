...
For CI or other non-interactive environments, set `COPILOT_GITHUB_TOKEN` to a GitHub token for a user with GitHub Copilot access. This is the only GitHub token environment variable Vekil consumes directly, and it overrides cached Vekil login state. Supported GitHub credentials are sent directly to Copilot: `gho_` OAuth user tokens, `ghu_` GitHub App user tokens, and `github_pat_` fine-grained PATs with the Copilot Requests permission. Resolving these credentials does not call the private `copilot_internal` token or user endpoints. The Copilot upstream authorizes the credential when Vekil probes `/models` or sends inference, so revoked, expired, unlicensed, or under-permissioned credentials surface as upstream authentication or permission failures.

Unless explicitly overridden, direct `ghu_` credentials use the `copilot-language-server` integration ID for `/models` and `/chat/completions`, keeping the Chat catalog aligned with inference authorization. Responses requests retain `vscode-chat`, which is required for the Responses-only models in that catalog. Other credential types use `vscode-chat` throughout.
