...
### Live dashboard apply and route generations

A supported long-lived loopback CLI/menubar server can edit providers and routes through `/dashboard/config`. The browser submits a complete non-secret document against the active ETag; the server restores protected fields and merges explicit secret operations from that exact base revision. Successful edits are written to a source-scoped managed JSON override rather than the selected JSON/YAML bootstrap. See [Local Dashboard Configuration](dashboard-config.md).

A hot apply builds a private candidate provider setup, performs required dynamic model discovery, compiles routes and policy bindings, persists the candidate, and then publishes one immutable runtime generation. The active route/catalog snapshot remains available until publication. A failed candidate cannot partially update `/v1/models`, route lookup, provider auth, policy selection, or readiness. After success, new requests and `/v1/models` use the new generation immediately; requests already admitted continue on their pinned old generation, and late old-generation discovery/cache work cannot populate the new generation.

Startup source precedence is bootstrap first, then a source-ID/digest-matching managed override. Changing the bootstrap semantically while its managed override remains is a hard conflict, not a merge. Use `--ignore-managed` for a read-only bootstrap start or `--reset-managed` to delete the matching override. The original bootstrap remains unchanged.

Stateful continuation uses both logical route/target identity and an internal physical target revision. Unrelated config changes can preserve compatible process-local state, but changes to destination, auth/credential identity, endpoint semantics, secret headers, upstream model, native endpoint contract, or relevant request policy invalidate old state for the new generation. Vekil fails those continuations locally and asks the client to restart the affected turn; it never migrates provider state or silently switches a bound continuation to a different target. Existing Responses WebSockets remain on their session-pinned old generation.
...
To stop automatic switching, restore `mode: primary_only`. On a supported local loopback server, the dashboard can apply that change without restarting: in-flight requests finish on their pinned generation and new requests use the newly published route. Compatible target revisions may preserve cross-request state; changed physical targets fail closed. When changing the bootstrap file or rolling a deployment, restart the same schema-version-2 binary. A restart is availability-safe but **not continuity-preserving**: it clears process-local state bindings, Responses-backed Chat replay state, and WebSocket sessions, so drain stateful traffic first or accept deterministic continuation failures.

Do **not** restore a version-1 file or downgrade to an older binary while state issued by a secondary may still be presented. First fence new stateful continuations, drain WebSocket sessions, wait at least the 24-hour binding TTL plus any longer provider replay window, and perform an atomic no-mixed-version cutover. If that fence cannot be guaranteed, keep the running version-2 process in `primary_only` until the fence is complete. There is no automatic configuration migration in either direction and no dashboard configuration-history/rollback system beyond reset to the bootstrap source.
