...
Remote sources are fetched once per server startup, validation command, or managed launcher invocation with a 15-second total timeout and a 4 MiB response-body limit. Vekil does not poll for changes. The URL path extension selects YAML; query parameters do not affect format detection, and other extensions use JSON decoding. Prefer HTTPS and use environment-backed credentials instead of putting secrets in a remotely hosted config. Any fetch, non-2xx response, decode, or validation failure stops startup.
