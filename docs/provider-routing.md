...
Unless explicitly overridden, direct `ghu_` credentials use the `copilot-language-server` integration ID for catalog and inference requests so discovered models and authorization use the same integration identity. Other credential types use `vscode-chat`.
