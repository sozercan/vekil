...
Use this file when editing provider credentials, model ownership, JSON/YAML provider configs, provider header profiles, route exposure, or provider-specific model metadata. For schema-v3 classifier policy, privacy, and rollout details, see [Semantic Policy Routing](policy-routing.md). For where to obtain provider keys, see [Provider API Keys](provider-api-keys.md). For global flags and env vars, see [Configuration](configuration.md).
...
Schema version 3 preserves the existing version-1/version-2 behavior and adds:
...
Schema-v3-only fields are rejected in version-1/version-2 files. Internal routes have no public ID, aliases, picker metadata, `/v1/models` entry, dashboard insight-model eligibility, or direct client resolution. Policy destinations may be public or internal; exposed public destinations are deliberate policy bypasses. The recommended configuration keeps both destinations and the classifier internal. See [`examples/policy-routing-coding-economy.yaml`](../examples/policy-routing-coding-economy.yaml) and [Semantic Policy Routing](policy-routing.md).

### Route schema and validation

For a public version-2/version-3 route, required route fields are `id`, `public_id`, an explicit nonempty `endpoints` allowlist of verified native upstream operations, and at least one target. A version-3 internal route requires `id`, `exposure: internal`, endpoints, and targets but must omit `public_id` and picker metadata. Each target requires a route-local `id`, `provider`, and `upstream_model`. Provider, route, and target operational IDs in schema version 2 or 3 are limited to 128 bytes and restricted to bounded ASCII identifier characters. The initial implementation accepts at most 256 routes, 32 targets per route, and 1,024 explicit targets total.
...
| `trust_domain` | providers used by schema-v3 policy destinations/classifiers | Operator-defined data-governance domain. Required for every provider referenced by a policy; matching is enforced unless the profile acknowledges cross-domain forwarding. |
| `classifier_no_store_supported` | provider used by a schema-v3 classifier route | Declares that Vekil can send the provider's supported non-storage option for classifier requests. This is a capability declaration, not proof of retention behavior. |
