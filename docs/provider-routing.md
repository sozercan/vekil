...
- top-level `policy_profiles` that publish one Chat model ID and select a `lightweight` or `powerful` OpenAI-family Chat execution route.
...
An OpenAI-family route may combine Azure and static OpenAI-compatible targets only when all targets implement every advertised endpoint with equivalent semantics. An Anthropic-family route contains only static Anthropic-compatible targets. In the general public/unreferenced route matrix, Copilot, OpenAI Codex, dynamically discovered generic providers, native `/realtime`, and heterogeneous OpenAI/native-Anthropic target sets are rejected as explicit route targets. The sole Copilot exception is a pinned target on an internal route referenced by `policy_profiles`, as described below. Provider-only version-1 routing for those providers remains available.

Schema-v2 policy selection is narrower than this general explicit-route matrix. In v1, terminal and classifier routes must use supported OpenAI-family Chat execution through native `/chat/completions` or proxy-owned Chat-over-Responses. Pinned `type: copilot` targets may use dynamic discovery; other dynamic provider kinds remain ineligible. The policy public ID is accepted only by `POST /v1/chat/completions` with text and standard function tools. It is rejected on direct Responses HTTP/websocket, Anthropic, Gemini, count-token, compaction, memory, and multimodal public surfaces. Direct public routes keep the general matrix above.
