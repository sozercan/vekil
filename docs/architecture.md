...
Bindings use keyed digests. The default process-local index holds 262,144 entries with a 24-hour absolute TTL; expiry, eviction, restart or another process makes old proof unknown. Optional `state_binding_durable*` storage uses synchronous bbolt transactions, a persistent digest key and a single-writer lock. `route_state_durable_identity.go` fingerprints the authenticated request before dispatch; existing exposure hooks commit records before publishing state. Durable records do not expire or evict, and I/O uncertainty freezes the store until reopen. bbolt supplies the transaction, sync and file-lock boundary rather than a custom crash journal. Raw state is not a log field or metric label. See [State Recovery](state-recovery.md) for capacity, pruning, lifecycle and supported-platform limits. Shared storage and cross-target replay/session migration remain unsupported.
...
- The Codex websocket bridge is disabled by default and uses upstream HTTP `/responses` when enabled. A separate experimental native-upstream option reuses a Copilot Responses websocket while preserving target binding and forbidding resend after dispatch. Other providers keep the HTTP transport.
...
Replay-backed function calls prefer a self-describing `call_vekil_v1_<15-character-nonce>_<upstream-call-id>_<4-character-checksum>` ID. When that form would exceed Anthropic's 64-character limit or the upstream ID is not embeddable, Vekil mints the opaque `call_vekil_<22-character-base64url>` fallback. Clients must return either ID unchanged. The nonce separates groups that reuse one upstream call ID; the checksum provides integrity, not authorization. An Anthropic reasoning carrier contains encrypted reasoning, ordering placeholders, projection bindings, and public IDs, but no separate provider-owned call ID.

Resolution is bound to the captured provider, public model, upstream model, complete assistant content, and complete ordered tool-call projection. The assistant call array cannot be shortened or reordered, but a complete set of tool-result messages may arrive in any order. For a non-empty partial result set, the adapter restores only the matching prior function calls and outputs; this is an empirical compatibility requirement because replaying the complete parallel call group with only partial outputs was rejected upstream. Missing calls may be reissued.

Replay state is process-local and expires one hour after capture; reads do not extend that deadline. It is also bounded by group count, per-group bytes, total bytes, item count, and call count, with LRU eviction under pressure. A restart, expiry, eviction, forged ID, or route mismatch produces the deterministic `responses_replay_state_missing` client error rather than attempting to reconstruct hidden reasoning. The exception is Anthropic ingress **on a direct route**, which opts into rebuilding the turn, immediately on a single-target route and only once every candidate target has already refused it on a multi-target one. A valid reasoning carrier restores hidden reasoning. A self-describing ID restores the original upstream call mapping whether or not the carrier survived; an opaque ID rebuilds the call and output under the proxy ID. Client-held carrier signatures have a 2 MiB cumulative wire-byte ceiling; Vekil emits no new carrier when the existing transcript is at the ceiling or the next signature would cross it. Native `/v1/chat/completions` and `/v1/responses` take neither degradation path and keep the deterministic error. A policy profile also fails after a process change because its tier comes from a carrier tag keyed per process, so trial resolution fails in the planner before Anthropic fallback. Raw replay content is never logged.
