...
Provider-issued continuation state is an exact ownership constraint, not a routing hint. Adapter-marked values such as `previous_response_id`, trusted `X-Codex-Turn-State`, non-proxy opaque `encrypted_content`, and other opaque reasoning/session artifacts are bound to `{route_id, target_id, credential_id}` immediately before exposure. The credential component is empty for providers without an internal account pool.

All state inputs on a request must resolve together to the same execution owner. Known state pins the exact route, target, and—when present—credential generation and disables failover. Conflicting, malformed, cross-route, mixed known/unknown, or unknown state on an explicit route fails locally without an upstream call. If the same token is ever observed with different owners, the store records a conflict tombstone and continues to fail it closed rather than choosing one owner. An unavailable bound target or stale credential generation also fails closed; Vekil does not guess another owner or migrate provider state.
...
- An opt-in OpenAI Codex account pool remains internal to one provider. Provider-catalog origin stays distinct from managed execution: pooled Codex catalog routes use the managed route executor for send safety and exact state ownership, while ordinary catalog routes retain provider-owned execution. The pool owns scheduling, model eligibility, health, cooldowns, affinity, and credential generations; the route executor owns physical sends and failover admission.
