...
| GGUF URL | `https://example.com/model-Q4_K_M.gguf` | a runner image that downloads the file at startup |
...
case the routes define its public contract. The agent context floor does not
...
- Containers are named `vekil-aikit-<id>` and labeled `dev.vekil.aikit`. A later
  ...
  no longer running, except kept ones. Remove a kept container with
