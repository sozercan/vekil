...
By default, target/state bindings are process-local, capped at 262,144 entries, and use a 24-hour absolute TTL. Opt-in [durable state recovery](state-recovery.md) preserves exact ownership across restart but does not recover websocket history or upstream connections. Reconnect with full client-held input without the old connection-local `previous_response_id`; retained encrypted input must still match its owner. Use one writer or sticky ingress and drain sessions before rollback/downgrade. Shared bindings, signed target hints, and cross-target replay are not implemented.

| Flag | Env Var | Default | Description |
|------|---------|---------|-------------|
| `--responses-ws-enabled` | `RESPONSES_WS_ENABLED` | `false` | Enable the proxy-owned Codex websocket bridge on `GET /v1/responses` |
| `--responses-ws-native-upstream` | `RESPONSES_WS_NATIVE_UPSTREAM` | `false` | Experimental persistent upstream Copilot Responses websocket; requires the bridge to be enabled |
...
`response.create.input` accepts a string or an item array. A string becomes one
user input message. Per-turn `headers` accept supported attribution and client
metadata, with at most 32 entries, 8 KiB per value, and 16 KiB total. Credentials,
integration overrides, and server-issued continuation state are not caller
metadata. `stream_id` fails explicitly because sessions are serialized.
Multiplexing and request superseding are unsupported. Local `generate: false`
staging does not dispatch inference.

## Experimental native upstream transport

With both `--responses-ws-enabled` and `--responses-ws-native-upstream`,
Copilot-backed sessions can keep one upstream Responses websocket open. After
the first turn, continuations send new input with the last response ID, reducing
...
Local warm-transport benchmarks with a fixed 400-message history reduced
continuation uploads from 140,921 bytes to 176 bytes. This measures local
transport and payload work, not provider latency or quota savings. See
[benchmark coverage](development.md#native-responses-transport-benchmark).
