...
- Once a target is committed, the session is pinned to the exact `{route_id, target_id, target_revision}` in the runtime generation captured when the WebSocket was admitted. Every later turn, delta replay, full-replay fallback, automatic compaction call, and protocol recovery stays on that target. Cross-target or cross-generation session migration is not implemented.
...
Target/state bindings are process-local, capped at 32,768 entries, and use a 24-hour absolute TTL. The target revision is an internal credential-safe fingerprint of the physical destination/auth/endpoint/upstream-model/request-policy identity; changing that identity prevents reuse even when route and target IDs are unchanged.

A dashboard hot apply does not move an open WebSocket to the new runtime. The session continues on its pinned old generation and target until it closes; only new connections use the new generation. Websocket session history also lives in the process that owns the connection. Use one Vekil process or sticky ingress, and drain sessions before restart, rollback, or downgrade. A process restart or another replica cannot recover the live session or its binding map. Durable/shared bindings, signed target hints, and cross-target full replay are not implemented.
