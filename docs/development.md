...
- If the provider participates in policy routing, define and validate its `trust_domain`, classifier non-storage capability, forced function-tool support, and live-preflight behavior. V1 policy destinations remain OpenAI-family Chat execution routes. Pinned direct Copilot targets and Responses-backed Chat are supported; do not silently admit other dynamic provider kinds, Anthropic, Gemini, multimodal, or multi-tenant policy behavior.
