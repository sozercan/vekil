...
Paid live validation runs weekly or on explicit manual dispatch, not on every push or pull-request update. Core CI still runs the deterministic protocol, routing, auth, and process tests on every pull request. The credential-free Zen workflow remains automated. Provider changes can be checked before merge by manually dispatching the relevant live workflow on the task branch; direct-bearer validation remains restricted to `main`.

The generic Copilot scripts use explicit inexpensive model allowlists. A missing required model or capability fails before inference instead of selecting an arbitrary catalog model. Optional Gemini CLI coverage skips when no approved native-Chat Flash model is listed. The weekly policy workflow explicitly reserves `claude-opus-4.7` for its distinct high-effort secondary and keeps the exact Sol `low`/`max` regression test. Repository model overrides are deliberate wider-coverage choices and must still satisfy endpoint and effort constraints. See [GitHub's current model pricing](https://docs.github.com/en/copilot/reference/copilot-billing/models-and-pricing) before changing them.
...
The repository also includes a `Live Copilot Smoke` workflow in [`.github/workflows/live-copilot-smoke.yaml`](../.github/workflows/live-copilot-smoke.yaml). It runs weekly on Monday at 09:31 UTC and can also be started manually from the Actions tab. It does not run pull-request-controlled code automatically because the job exposes a live Copilot credential while building the checked-out revision and running repository scripts.
...
For the broad matrix, the wrapper requires native `/chat/completions` support and the required tier effort. Default selection is limited to:

- lightweight: `gpt-5-mini`, then `gpt-5.4-mini`, advertising `low`;
- classifier: `gpt-5-mini`, then `claude-haiku-4.5`;
- powerful primary: `gpt-5-mini`, then `gpt-5.4-mini`, advertising `high`; and
- powerful secondary: a distinct model advertising `high`. The weekly workflow explicitly pins `claude-opus-4.7` unless its repository override is set. The standalone wrapper only tries the two approved GPT mini models unless given an explicit override.
...
In the Copilot weekly/manual matrix, the powerful targets are distinct models but share one Copilot service and loopback bridge. That proves sealed tier selection, retry accounting, target switching, and public-identity behavior; it does **not** prove independent cross-provider availability.
...
Both workflows use isolated loopback ports, reject port `1337`, retry bounded address-in-use races, and verify that every proxy, bridge, and shim listener is released. Do not change either workflow to `pull_request_target`: executing pull-request code with provider credentials would expose secrets to untrusted code. Neither paid policy workflow runs automatically on pull requests. A Copilot run without `COPILOT_GITHUB_TOKEN` fails instead of looking like completed coverage. The manual multi-provider workflow fails when its configuration is missing.
...
Run the Copilot-backed matrix locally after `make build`, explicitly choosing its second high-effort model:

```bash
COPILOT_GITHUB_TOKEN=... \
  LIVE_POLICY_ROUTING_COPILOT_POWERFUL_SECONDARY_MODEL=claude-opus-4.7 \
  scripts/live-policy-routing-copilot-smoke.sh
```
...
The [`Live OpenCode Zen Smoke`](../.github/workflows/live-zen-smoke.yaml) workflow runs the **same** `scripts/live-cli-smoke.sh` harness as the Copilot smoke, but in `SMOKE_PROVIDER=zen` mode: it starts vekil with `examples/opencode-zen-free.yaml` (no credentials) and drives real coding-agent CLIs against the OpenCode Zen free tier. Because it needs no secrets, it runs on **every** pull request, **including external-contributor forks**. Paid Copilot checks run separately on weekly/manual triggers. It is the only live end-to-end coverage of vekil's generic `openai-compatible` provider routing (config loading, bearer auth, static model catalog, and the per-model endpoint allowlist), which zero-config Copilot startup never exercises.
...
4. Run either workflow from the Actions tab when needed. Both also run weekly from `main`; neither runs automatically on pushes or pull requests.
...
The credentialed Copilot workflows remain separate from deterministic core CI and fail when their required credential is unavailable. The direct-bearer workflow is restricted to `main` as described above.
