...
| [`provider-routing.md`](provider-routing.md) | provider auth, schema-v2 model routes, ordered priority failover, model ownership, endpoint allowlists | providers, routing behavior, failover safety, auth, or model metadata changes |
...
| [`architecture.md`](architecture.md) | package responsibilities, route registry/executor, replay safety, and data flow | implementation boundaries or design decisions change |
| [`dashboard.md`](dashboard.md) | live browser traffic dashboard, route/attempt metrics, and `/stats.json` | dashboard metrics, endpoints, or stats behavior change |
| [`menubar.md`](menubar.md) | macOS/Linux tray app usage | tray behavior or packaging changes |
| [`development.md`](development.md) | build, test, route-safety matrix, benchmark baseline, and CI workflows | local dev, test gates, benchmark commands, or CI change |
