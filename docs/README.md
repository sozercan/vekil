...
| [`provider-routing.md`](provider-routing.md) | provider auth, schema-v2 model routes, ordered priority failover, model ownership, native endpoint allowlists | providers, routing behavior, failover safety, auth, or model metadata changes |
...
| [`metrics.md`](metrics.md) | Prometheus `/metrics` endpoint, metric names, and scraping | metrics or observability behavior changes |
| [`menubar.md`](menubar.md) | macOS/Linux tray app usage | tray behavior or packaging changes |
| [`development.md`](development.md) | build, test, route-safety and Chat-over-Responses matrices, benchmark baselines, and CI workflows | local dev, test gates, benchmark commands, or CI change |
