...
When `model` resolves to a schema-v2 policy profile, Vekil applies the policy before normal route execution. V1 policy requests must be text-only and may use only standard function tools. Policy destinations may expose native `/chat/completions` or proxy-owned Chat-over-Responses; the policy public ID itself remains a Chat-only surface. Unsupported content or fields fail locally instead of falling through to a direct route.
